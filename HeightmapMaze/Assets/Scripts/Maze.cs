...
public class Maze : MonoBehaviour 
{
    ...
    {
        // Load the level map
        Texture2D levelMap = (Texture2D)Resources.LoadAssetAtPath("Assets/Levels/Level2.bmp", typeof(Texture2D));
        ...
        for (int i = 0; i < levelMap.width; i++)
        {
            for (int j = 0; j < levelMap.height; j++)
            {
                ...
                cells[i, j].transform.localPosition = new Vector3(i, -levelMap.GetPixel(i, j).grayscale * cells[i, j].transform.localScale.y, j);
            }
        }
    }
